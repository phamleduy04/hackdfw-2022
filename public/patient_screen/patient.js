...
function loadDoctor(roomID, array) {
    $.get(`/data?roomID=${roomID}`, (data) => {
        ...
        $('#docPic > div > div.col-md-8 > div > p:nth-child(2)').append(`ID: ${firstDoc.id}`);
        $('#docPic > div > div.col-md-8 > div > p:nth-child(3)').append(`Department : ${firstDoc.department}`);
        $('#docPic > div > div.col-md-8 > div > p:nth-child(4)').append(`Postion : ${firstDoc.position}`);
